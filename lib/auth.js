...
var OAUTH_BASE = 'https://mockflow.com';
var OAUTH_AUTHORIZE = OAUTH_BASE + '/mcp-oauth/';
...
function login() {
	...
	var callbackServer = http.createServer(function(req, res) {
		...
		exchangeCodeForToken(code, function(err, tokenData) {
			...
			validateToken(tokenData.access_token, function(err2, userInfo) {
				...
				console.log('  mockflow-wireframepro-mcp');
				console.log('');
	...
	callbackServer.on('error', function(err) {
		if (err.code === 'EADDRINUSE') {
			console.log('Port ' + CALLBACK_PORT + ' is in use. Close any running mockflow-wireframepro-mcp login and try again.');
		} else {
