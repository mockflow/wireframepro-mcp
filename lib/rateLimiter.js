...
function createRateLimiter(opts) {
	...
	var windowSec = Math.round(WINDOW_MS / 1000);
	...
	function middleware(req, res, next) {
		...
		if (remaining <= 0) {
			...
			var useSSE = accept.indexOf('text/event-stream') !== -1;
			...
			var resultBody = {
				...
				result: {
					content: [
						{
							type: 'text',
							text: 'Rate limit reached: maximum ' + LIMIT + ' tool calls per ' + windowLabel + '. Please wait ' + resetSeconds + ' seconds before trying again.'
						}
					],
					isError: true
				}
			...
			if (useSSE) {
				...
				res.write('data: ' + JSON.stringify(resultBody) + '\n\n');
				res.end();
			} else {
				res.json(resultBody);
			}
