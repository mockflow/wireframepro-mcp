...
async function start(port, options) {
	...
	console.log('  Example: Create a wireframe of login form using wireframepro cli.');		
	console.log('');
	...
	console.log('  Works with your codebase and project context.');
	console.log('  Example: "Based on my website repo create a pricing page wireframe using same styleguide"');
	console.log('');
	console.log('Skills:');
...
async function proxyToBackend(toolName, args, creds, spaceId) {
	...
	// Production: https://app.mockflow.com/wire/{projectId}#/page/{pageId}/mode/design
	// Local dev:  http://localhost:8080/MockFlow-WireframePro/editor.jsp?projectid={projectId}#/page/{pageId}/mode/design
	if (payload.projectUrl) {
		if (process.env.MCP_DEBUG) console.log('[WP-MCP] projectUrl received:', payload.projectUrl);
		var urlMatch = payload.projectUrl.match(/\/wire\/([A-Za-z0-9]+)/) || payload.projectUrl.match(/projectid=([A-Za-z0-9]+)/);
		...
		if (process.env.MCP_DEBUG) console.log('[WP-MCP] Extracted _projectId:', payload._projectId || 'NONE', '_pageId:', payload._pageId || 'NONE');
		delete payload.projectUrl;
	}

	// In dev mode, pass username for local auth bypass
	if (process.env.MCP_DEV && creds.userid) {
		payload._devUser = creds.userid;
	}
