...
function start(port, options) {
	...
	if (!creds || (!creds.access_token && !creds.apiKey)) {
		console.log('');
		console.log('No credentials found. Run "mockflow-mcp login" or "mockflow-wireframepro-mcp login" first.');
		console.log('');
		process.exit(1);
	}
	...
	var rateLimiter = createRateLimiter({ limit: 20, windowMs: 60 * 1000 });
